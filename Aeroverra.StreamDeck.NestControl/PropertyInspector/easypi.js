...
function openWebsite() {
    openExternalUrl('https://aerove.aero.vi/streamdeck/nestcontrol');
}

function openHelp() {
    openExternalUrl('https://github.com/Aeroverra/StreamDeck-Nest-Control/issues');
}

function openExternalUrl(url) {
    if (!websocket || websocket.readyState !== 1) {
        console.warn('Cannot open url, websocket not ready', url);
        return;
    }
